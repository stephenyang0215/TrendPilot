...
async function fetchFromAzureStorage(containerName, blobPath) {
...
function parseCSV(csvContent, isHistorical = true) {
  ...
  const priceCol = isHistorical ? headers.findIndex((h)=>h === 'c') : headers.findIndex((h)=>h === 'pred_price');
...
function calculateMetrics(historical, forecast) {
  ...
  const firstPrice = historical[historical.length - 36]?.price;
  console.log(`currentPrice ${currentPrice}`);
  console.log(`forecastPrice ${forecastPrice}`);
